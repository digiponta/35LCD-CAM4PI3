...
sizeData = [ # Camera parameters for different size settings
 ...
 [(2592, 1944), (480, 320), (0.0, 0.0 , 1.5, 1.33 )], # Large
 [(1920, 1080), (480, 270), (0.1944, 0.2962, 1.1111, 0.7408)], # Med
 [(1440, 1080), (480, 320), (0.3333, 0.2962, 0.8334, 0.7408)]] # Small
isoData = [ # Values for ISO settings [ISO value, indicator X position]
 [  0,  27*1.5], [100,  64*1.5], [200,  97*1.5], [320, 137*1.5],
 [400, 164*1.5], [500, 197*1.5], [640, 244*1.5], [800, 297*1.5]]
...
buttons = [
  ...
  [Button((   0, 250, 480,  69), bg='done' , cb=doneCallback),
   Button((   0,   0, 120,  69), bg='prev' , cb=imageCallback, value=-1),
   Button(( 320,   0, 120,  69), bg='next' , cb=imageCallback, value= 1),
   Button(( 117,  93, 235, 136)), # 'Working' label (when enabled)
   Button(( 222, 172,  33,  29)), # Spinner (when enabled)
   Button(( 182,   0, 117,  69), bg='trash', cb=imageCallback, value= 0)],
  # Screen mode 1 is delete confirmation
  [Button((  0, 35*1.33,320*1.5, 33*1.33), bg='delete'),
   Button(( 32*1.5,86*1.33,120*1.5,100*1.33), bg='yn', fg='yes',
    cb=deleteCallback, value=True),
   Button((168*1.5,86*1.33,120*1.5,100*1.33), bg='yn', fg='no',
    cb=deleteCallback, value=False)],
  # Screen mode 2 is 'No Images'
  [Button((0*1.5,  0*1.33,320*1.5,240*1.33), cb=doneCallback), # Full screen = button
   Button((0*1.5,188*1.33,320*1.5, 52*1.33), bg='done'),       # Fake 'Done' button
   Button((0*1.5, 53*1.33,320*1.5, 80*1.33), bg='empty')],     # 'Empty' message
  # Screen mode 3 is viewfinder / snapshot
  [Button((  0,  250, 234,  69), bg='gear', cb=viewCallback, value=0),
   Button(( 246, 250, 234,  69), bg='play', cb=viewCallback, value=1),
   Button((   0,  0,  480, 320)           , cb=viewCallback, value=2),
   Button(( 132, 68,  236, 136)),  # 'Working' label (when enabled)
   Button(( 222, 145,  33, 29))], # Spinner (when enabled)
  # Remaining screens are settings modes
  # Screen mode 4 is storage settings
  [Button((  0*1.5,188*1.33,320*1.5, 52*1.33), bg='done', cb=doneCallback),
   Button((  0*1.5,  0*1.33, 80*1.5, 52*1.33), bg='prev', cb=settingCallback, value=-1),
   Button((240*1.5,  0*1.33, 80*1.5, 52*1.33), bg='next', cb=settingCallback, value= 1),
   Button((  2*1.5, 60*1.33,100*1.5,120*1.33), bg='radio3-1', fg='store-folder',
    cb=storeModeCallback, value=0),
   Button((110*1.5, 60*1.33,100*1.5,120*1.33), bg='radio3-0', fg='store-boot',
    cb=storeModeCallback, value=1),
   Button((218*1.5, 60*1.33,100*1.5,120*1.33), bg='radio3-0', fg='store-dropbox',
    cb=storeModeCallback, value=2),
   Button((  0*1.5, 10*1.33,320*1.5, 35*1.33), bg='storage')],
  # Screen mode 5 is size settings
  [Button((  0*1.5,188*1.33,320*1.5, 52*1.33), bg='done', cb=doneCallback),
   Button((  0*1.5,  0*1.33, 80*1.5, 52*1.33), bg='prev', cb=settingCallback, value=-1),
   Button((240*1.5,  0*1.33, 80*1.5, 52*1.33), bg='next', cb=settingCallback, value= 1),
   Button((  2*1.5, 60*1.33,100*1.5,120*1.33), bg='radio3-1', fg='size-l',
    cb=sizeModeCallback, value=0),
   Button((110*1.5, 60*1.33,100*1.5,120*1.33), bg='radio3-0', fg='size-m',
    cb=sizeModeCallback, value=1),
   Button((218*1.5, 60*1.33,100*1.5,120*1.33), bg='radio3-0', fg='size-s',
    cb=sizeModeCallback, value=2),
   Button((  0*1.5, 10*1.33,320*1.5, 29*1.33), bg='size')],
  # Screen mode 6 is graphic effect
  [Button((  0*1.5,188*1.33,320*1.5, 52*1.33), bg='done', cb=doneCallback),
   Button((  0*1.5,  0*1.33, 80*1.5, 52*1.33), bg='prev', cb=settingCallback, value=-1),
   Button((240*1.5,  0*1.33, 80*1.5, 52*1.33), bg='next', cb=settingCallback, value= 1),
   Button((  0*1.5, 70*1.33, 80*1.5, 52*1.33), bg='prev', cb=fxCallback     , value=-1),
   Button((240*1.5, 70*1.33, 80*1.5, 52*1.33), bg='next', cb=fxCallback     , value= 1),
   Button((  0*1.5, 67*1.33,320*1.5, 91*1.33), bg='fx-none'),
   Button((  0*1.5, 11*1.33,320*1.5, 29*1.33), bg='fx')],
  # Screen mode 7 is ISO
  [Button((  0,188*1.33,320*1.5, 52*1.33), bg='done', cb=doneCallback),
   Button((  0,  0*1.33, 80*1.5, 52*1.33), bg='prev', cb=settingCallback, value=-1),
   Button((240*1.5,  0*1.33, 80*1.5, 52*1.33), bg='next', cb=settingCallback, value= 1),
   Button((  0, 70*1.33, 80*1.5, 52*1.33), bg='prev', cb=isoCallback    , value=-1),
   Button((240*1.5, 70*1.33, 80*1.5, 52*1.33), bg='next', cb=isoCallback    , value= 1),
   Button((   0, 105, 427, 44 ), bg='iso-0'),
   Button((  13, 177, 453, 35 ), bg='iso-bar'),
   Button((  26, 209,  32, 25 ), bg='iso-arrow'),
   Button((   0,  13, 480, 39 ), bg='iso')],
  # Screen mode 8 is quit confirmation
  [Button((  0*1.5,188*1.33,320*1.5, 52*1.33), bg='done'   , cb=doneCallback),
   Button((  0*1.5,  0*1.33, 80*1.5, 52*1.33), bg='prev'   , cb=settingCallback, value=-1),
   Button((240*1.5,  0*1.33, 80*1.5, 52*1.33), bg='next'   , cb=settingCallback, value= 1),
   Button((110*1.5, 60*1.33,100*1.5,120*1.33), bg='quit-ok', cb=quitCallback),
   Button((  0*1.5, 10*1.33,320*1.5, 35*1.33), bg='quit')]
]
